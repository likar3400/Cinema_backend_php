        ...
        <?php foreach ($bookings as $b): ?>
            <div class="booking-card <?= $b['status']==='cancelled'?'bk-cancelled':'' ?>" id="bk-<?= $b['id'] ?>">
                <div class="bk-header">
                    ...
                    <span class="status-badge st-<?= $b['status'] ?>">
        <?= $b['status']==='confirmed' ? '✅ Активний' : '❌ Скасовано' ?>
      </span>
                </div>
                <div class="bk-body">
                ...
                <?php if ($b['status']==='confirmed' && strtotime($b['starts_at']) > time()): ?>
                    <div class="bk-footer">
                        <button class="btn btn-danger btn-sm" onclick="cancelBk(<?= $b['id'] ?>)">
        ...
        fetch('<?= APP_URL ?>/api/booking/cancel', {
            ...
            .then(d => {
                if (d.success) {
                    ...
                    alert('Помилка: ' + (d.message || 'невідома'));
                }
            })
